...
<div class="container container-single-page penci-breadcrumb">
			<span><a class="crumb" href="<?php echo esc_url( home_url('/') ); ?>"><?php echo penci_get_setting( 'penci_trans_home' ); ?></a></span> / 
			<?php
			...
			if( ! empty( $page_parent ) ):
				...
				foreach( $page_parent as $pages ){
					?>
					<span><a class="crumb" href="<?php echo get_permalink( $pages ); ?>"><?php echo get_the_title( $pages ); ?></a></span>/
				<?php }
			endif; ?>
		...
		<?php if ( ! get_theme_mod( 'penci_single_meta_author' ) || ! get_theme_mod( 'penci_single_meta_date' ) || ! get_theme_mod( 'penci_single_meta_comment' ) || get_theme_mod( 'penci_single_show_cview' ) ) : ?>
			<div class="post-box-meta-single">
				<?php if ( ! get_theme_mod( 'penci_single_meta_author' ) ) : ?>
					<span class="author-post byline"><span class="author vcard"><?php echo __('written by','soledad'); ?> <a class="author-url url fn n" href="<?php echo get_author_posts_url(get_the_author_meta( 'ID' )); ?>" ><?php echo get_the_author_meta( 'first_name' ).' '.get_the_author_meta( 'last_name' ); ?></a> </span></span>
				<?php endif; ?>
			...
			<div class="post-image">
				<div class="penci-owl-carousel penci-owl-carousel-slider penci-nav-visible" data-auto="<?php echo $autoplay; ?>" data-lazy="true">
					<?php foreach ( $images as $image ) : ?>
						...
						?>
						<a href="<?php echo esc_url( $the_image[0] ); ?>" data-rel="penci-gallery-image-content">
							<figure>
								<?php if( ! get_theme_mod( 'penci_disable_lazyload_single' ) ) { ?>
	...
	<?php if( get_theme_mod( 'penci_move_title_bellow' ) && 'style-2' != $single_style ): ?>
		...
		<div class="header-standard header-classic single-header">
			...
			<?php if ( ! get_theme_mod( 'penci_single_meta_author' ) || ! get_theme_mod( 'penci_single_meta_date' ) || ! get_theme_mod( 'penci_single_meta_comment' ) || get_theme_mod( 'penci_single_show_cview' ) ) : ?>
				<div class="post-box-meta-single">
					<?php if ( ! get_theme_mod( 'penci_single_meta_author' ) ) : ?>
						<span class="author-post byline"><span class="author vcard"><?php echo __('written by','soledad'); ?> <a class="author-url url fn n" href="<?php echo get_author_posts_url( get_the_author_meta( 'ID' ) ); ?>"><?php the_author(); ?></a></span></span>
					<?php endif; ?>
					<?php if ( ! get_theme_mod( 'penci_single_meta_date' ) ) : ?>
						<span><?php penci_soledad_time_link(); ?></span>
					<?php endif; ?>
	...
	<div class="post-entry <?php echo 'blockquote-'. $block_style; ?>">
		<div class="inner-post-entry entry-content" id="penci-post-entry-inner">
			<?php 
				...
				$tempCheck = '[penci_recipe';

				$tempVerify = strpos($tempContent,$tempCheck);
				if($tempVerify == false) {
					echo do_shortcode('[penci_recipe]');
				}

				if( have_rows('produits') ):
			?>
					<div class="produitmonbento">
						<h3 class="penci-recipe-title"><?php _e('Presented products', 'soledad'); ?></h3>
						<div class="productspres">
							<?php 
								while ( have_rows('produits') ) : the_row();
								$prod = get_sub_field('produit');
								if($prod){
							?>
									<div class="productpres">
										<img src="<?php the_field('image_produit',$prod->ID); ?>" alt="productimage" />
										<span class="titleprod"><?php the_field('titre_produit',$prod->ID); ?></span>
										<span class="subtitleprod"><?php the_field('sous_titre_produit',$prod->ID); ?></span>
										<a class="productlink" target="_blank" href="<?php the_field('url_produit',$prod->ID); ?>"><?php the_field('libelle_bouton',$prod->ID); ?></a>
