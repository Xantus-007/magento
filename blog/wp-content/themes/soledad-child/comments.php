...
<div class="post-comments<?php if( $comment_numbers == 0 ): echo ' no-comment-yet'; endif;?>" id="comments">
	...
	if( ! get_theme_mod( 'penci_single_hide_save_fields' ) && version_compare( get_bloginfo('version'),'4.9.6', '>=' ) ){
		$fields['cookies'] = '<p class="comment-form-cookies-consent"><input id="wp-comment-cookies-consent" name="wp-comment-cookies-consent" type="checkbox" value="yes"' . $consent . ' />' .
							'<label class="comment-form-cookies-text" for="wp-comment-cookies-consent">' . __( 'Save my name, email, and website in this browser for the next time I comment.', 'soledad' ) . '</label></p>';
	}
	...
	$messagegdpr= "";
	$messagegdpr = __('By using this form, you agree to the storage and processing of your data by Monbento','soledad').'.';

	$gdrp_mess = '<p class="comment-form-cookies-consent"><input id="wp-comment-gdpr" name="wp-comment-gdpr" type="checkbox" ><span class="comment-form-cookies-text" for="wp-comment-gdpr">'.$messagegdpr.'</span></p>';
	// $messagegdpr = __('J’accepte de recevoir des newsletters de la part de monbento et que mes données seront traitées conformément à la politique de gestion des données personnelles de monbento. Je pourrai me désabonner à tout moment.');
