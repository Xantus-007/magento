...
if( get_theme_mod( 'penci_page_default_template_layout' ) == 'left-sidebar' ){
			...
			<div class="theiaStickySidebar">
				<?php if (have_posts()) : while (have_posts()) : the_post(); ?>
                    <?php get_template_part('content', 'page');
                        $at_least = 1;  // at least 1 articles in current category or tags
                    ?>
                    <div class="Authorslisting">
                        <?php

                            $author_array = list_author_in_this_cat ($at_least);
                            $nb_elem_per_page = 6;
							$page = get_query_var( 'paged' ) ? intval( get_query_var( 'paged' ) ) : 1;
							// die(var_dump(get_query_var( 'paged' )));
                            $data = (array)$author_array;
                            $number_of_pages = intval(count($data)/$nb_elem_per_page)+1;
                            foreach (array_slice($data, ($page-1)*$nb_elem_per_page, $nb_elem_per_page) as $author) : 
                        ?>

                        <div class="authorMonbento">
                            <?php
                                    set_query_var( 'ID', absint( $author ) );
					...
					<?php
                       if($number_of_pages>1 && count($data)/$nb_elem_per_page != 1){
                    ?>
                        <div class="penci-pagination">
                            <div class="newer">
                                <?php if( ($page+1) <= $number_of_pages && $page != ceil(count($data)/$nb_elem_per_page) ) { ?>
                                    <?php echo '<a href="./?paged='.($page+1).'"><span><i class="fa fa-angle-left"></i>'. __('Older Posts','soledad') .'</span></a>'; ?>
                                <?php } else { ?>
                                    <?php echo '<div class="disable-url"><span><i class="fa fa-angle-left"></i>'. __('Older Posts','soledad') .'</span></div>'; ?>
                                <?php } ?>
							</div>
							<div class="older">
                                <?php if( $page == 1 ) { ?>
                                    <?php echo '<div class="disable-url"><span>'. __('Newer Posts','soledad') .'<i class="fa fa-angle-right"></i></span></div>'; ?>
                                <?php } else { ?>
                                    <?php echo '<a href="./?paged='.($page-1).'"><span>'. __('Newer Posts','soledad') .'<i class="fa fa-angle-right"></i></span></a>'; ?>
                                <?php } ?>
                            </div>
                        </div>
                    <?php
