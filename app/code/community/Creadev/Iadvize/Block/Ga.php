    ...
    protected function _toHtml()
    {
				...
				if ($module == 'catalog') {
						if ($curProduct = Mage::registry('current_product')) {
								if ($curProduct->getIadvize(false)) $disp = true;
						}
						elseif ($curCategory = Mage::registry('current_category')){
								if ($curCategory->getIadvize(false)) $disp = true;
						}
				}
				elseif($module == 'cms' && $this->getHelper('cms/page')->getPage()->getData('iadvize', false) == true) {
						$disp = true;
				}
				elseif($module == 'checkout' || $module == 'customer') {
						$disp = true;
				}
