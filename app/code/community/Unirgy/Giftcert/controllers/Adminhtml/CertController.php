        ...
        if($file) {
            try {
                ...
                if(!is_readable($import_file)) {
                    Mage::throwException("File " . $file . " cannot be found in " . $target);
                }
                ...
                Mage::log($e->getMessage(), Zend_Log::ERR, 'giftcert.log', true);
                Mage::log($e->getTraceAsString(), Zend_Log::ERR, 'giftcert.log', true);
                $session->addException($e, $this->__('An error occurred while importing:').' '.$e->getMessage());
            }
        }
...
//        echo __METHOD__;
        ...
        if(!$gridBlock){
            ...
            $gridBlock = $layout->createBlock('ugiftcert/adminhtml_report', "ugiftcert.report.grid.container");
            /* @var $filterFormBlock Unirgy_Giftcert_Block_Adminhtml_Report_Form */
            $filterFormBlock = $layout->createBlock("ugiftcert/adminhtml_report_form", "grid.filter.form");
            $filterFormBlock->addReportTypeOption('created_at_order', $this->__("Order Created Date") );
            $filterFormBlock->addReportTypeOption('updated_at_order', $this->__("Order Updated Date") );

            $filterFormBlock->setFieldOption('report_type', 'note',
                  $this->__("Order Updated Date report is real-time, does not need statistics refreshing."));

            $gridBlock->append($filterFormBlock);
            if($content){
                $content->append($gridBlock, 'adminhtml_report.grid');
            }
        } else {
