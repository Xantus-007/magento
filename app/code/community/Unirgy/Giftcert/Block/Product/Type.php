...
class Unirgy_Giftcert_Block_Product_Type extends Mage_Catalog_Block_Product_View_Abstract
{
    ...
    {
        $options = explode(',', $this->getAddressOption('address_recipient_options'));
        if( !$this->getAllowPhysical() && $key = array_search(Unirgy_Giftcert_Model_Source_Address::SHIPPING, $options) ){
            unset($options[$key]);
        }
    ...
    {
        return explode(',',  $this->getAddressOption('address_sender_options'));
    }

    protected function getAddressOption($addressType)
    {
        $opt = Mage::getStoreConfig('ugiftcert/address/' . $addressType);
        if(empty($opt)){
            $opt = Unirgy_Giftcert_Model_Source_Address::CUSTOM;
        }
        return $opt;
    }
    ...
    {
        return Unirgy_Giftcert_Model_Source_Address::getOptionsStatic();
    }
}
