...
//        $result = parent::prepareForCart($buyRequest, $product);
        ...
        if ($multiple = $buyRequest->getData('multiple-recipients')) {
            ...
            $result = parent::prepareForCart($buyRequest, $product, Mage_Catalog_Model_Product_Type_Abstract::PROCESS_MODE_FULL);
        }
...
//        $amountConfig = $hlp->getAmountConfig($product);
            ...
            if ($v = $buyRequest->getData($p)) {
                switch ($p) {
                    ...
                    case 'sender_email':
                        ...
                        break;
                    case 'sender_use_address':
                    case 'recipient_use_address':
                        $addr_options = Unirgy_Giftcert_Model_Source_Address::getOptionsStatic();
                        if (!isset($addr_options[$v])) {
                            return Mage::helper('ugiftcert')->__('Supplied address type is invalid.');
                        }
                }
                $options[$k] = $v;
            }
        ...
        if (!isset($options['recipient_use_address']) || $options['recipient_use_address'] == Unirgy_Giftcert_Model_Source_Address::CUSTOM) {
            if ($options[self::DELIVERY_TYPE] == self::DELIVERY_TYPE_PHYSICAL && !isset($options['recipient_address'])) {
    ...
    protected function _getFields()
    {
        ...
        $fields['sender_use_address']    = 'sender_use_address';
        $fields['recipient_use_address'] = 'recipient_use_address';
        return $fields;
    }
