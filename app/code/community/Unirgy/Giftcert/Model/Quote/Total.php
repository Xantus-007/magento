...
class Unirgy_Giftcert_Model_Quote_Total extends Mage_Sales_Model_Quote_Address_Total_Abstract
{
    ...
    public function collect(Mage_Sales_Model_Quote_Address $address)
    {
        ...
        if ( empty( $qBaseBalances ) ) {
            ...
            foreach ( $qBaseBalances as &$bb ) {
                $bb = 0;
            }
            unset( $bb );
            $qBalances = $qBaseBalances;
        } else {
            while ( sizeof( $qBaseBalances ) < sizeof( $qFinalCertCodes ) ) {
                array_push( $qBaseBalances, 0 );
            }
            while ( sizeof( $qBalances ) < sizeof( $qFinalCertCodes ) ) {
                array_push( $qBalances, 0 );
            }
