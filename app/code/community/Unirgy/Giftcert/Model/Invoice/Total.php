...
//        }
        ...
        if (!$baseGcAmount || $baseGcAmount < 0.01 || $baseGcInvoiced==$baseGcAmount) {
            return $this;
        }
        ...
        if ($baseGcBalance >= $baseInvoiceTotal) {
            ...
            $baseInvoiceTotal = 0;
            $invoiceTotal = 0;
            $order->setTotalPaid($gcUsed);
            $order->setBaseTotalPaid($baseGcUsed);
        } else {
            ...
            $order->setTotalPaid($order->getTotalPaid() + $gcUsed);
            $order->setBaseTotalPaid($order->getBaseTotalPaid() + $baseGcUsed);
        }
