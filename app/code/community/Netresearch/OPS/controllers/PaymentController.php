...
class Netresearch_OPS_PaymentController extends Netresearch_OPS_Controller_Abstract
{
    ...
    public function generateHashAction()
    {
        ...
        $data = array(
            'ACCEPTURL'     => $config->getAliasAcceptUrl(),
            'ALIAS'         => $this->_request->getParam('alias'),
            'BRAND'         => $this->_request->getParam('brand'),
            'EXCEPTIONURL'  => $config->getAliasExceptionUrl(),
    ...
    {
        $alias = $this->_request->getParam('alias');
        // Mage::log('Ogone PaymentController saveAliasAction', null, 'ops_alias.log');
        if (0 < strlen($alias)) {
            $payment = $this->_getCheckout()->getQuote()->getPayment();
            ...
            // Mage::log($this->_getCheckout()->getQuote()->getPayment()->getAdditionalInformation(), null, 'ops_alias.log');
