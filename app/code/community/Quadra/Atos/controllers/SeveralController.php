 ...
 * @category   Quadra
 * @package    Quadra_Atos
 * @name       Quadra_Atos_SeveralController
 * @author     Quadra Team
 */

class Quadra_Atos_SeveralController extends Mage_Core_Controller_Front_Action
{	
    /**
     * Get singleton with Atos aurore
     *
     * @return object Quadra_Atos_Model_Several
     */
    ...
    public function automaticAction() 
    {
        ...
        if ($response['return_context']) {
            ...
            foreach ($orderIds as $orderId) {
                ...
                $this->_updateOrderState($order, $response);
            }
        } else {
            $order = Mage::getModel('sales/order');
            $order->loadByIncrementId($response['order_id']);
    ...
    protected function _updateOrderState($order, $response) {
        switch ($response['response_code']) {
            // Success order
            case '00':
                if ($order->getId()) {
    ...
    protected function saveInvoice(Mage_Sales_Model_Order $order)
    {
        if ($order->canInvoice()) {
            ...
            $order->addStatusToHistory(
                    Mage_Sales_Model_Order::STATE_PROCESSING, Mage::helper('atos')->__('Invoice %s was created', $invoice->getIncrementId())
            );
        }
