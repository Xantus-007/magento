...
class Wizkunde_ConfigurableBundle_Block_Catalog_Product_View_Type_Bundle extends Mage_Bundle_Block_Catalog_Product_View_Type_Bundle
{
    ...
    protected function _getConfigurableProducts($_selection)
    {
        ...
        $options = array();
        foreach ($pr->getTypeInstance(true)->getUsedProducts(null, $pr) as $product) {
    ...
    protected function _getConfigurableOptions($_selection)
    {
        ...
        Mage::dispatchEvent(
            ...
            array('product' => $this->getProduct(), 'qty' => 1, 'price' => $pr->getFinalPrice(), 'selection' => $pr)
        );
    ...
    protected function _preparePrice($price, $isPercent=false, $product)
    {
        ...
        Mage::dispatchEvent(
            'catalog_product_get_bundle_discount_price',
            array('product' => $this->getProduct(), 'qty' => 1, 'price' => $price)
        );
    ...
    public function getJsonConfig()
    {
        ...
        foreach ($optionsArray as $_option) {
            ...
            foreach ($_option->getSelections() as $_selection) {
                ...
                Mage::dispatchEvent(
                    'catalog_product_get_bundle_discount_price',
                    array('product' => $this->getProduct(), 'qty' => 1, 'price' => $_selection->getFinalPrice(), 'selection' => $_selection)
                );
