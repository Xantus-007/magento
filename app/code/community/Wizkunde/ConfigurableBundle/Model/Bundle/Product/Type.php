            ...
            foreach ($selections as $selection) {
                ...
                if($selection->getTypeId() == 'configurable') {
                    ...
                    $simpleProduct = Mage::helper('configurablebundle/bundle')->getSimpleProductByAttributes(
                        $selection,
                        $buyRequestData['super_attribute'][$selection->getEntityId()]
                    );

                    $loadedProduct = Mage::getModel('catalog/product')->load($simpleProduct->getId());
                    $price = $simpleProduct->getPriceModel()->getFinalPrice($qty, $loadedProduct);
                    $uniqueKey[] = $simpleProduct->getId() . $qty;
                } else {
    ...
    public function getSku($product = null)
    {
        ...
        if ($this->getProduct($product)->getData('sku_type')) {
            ...
            if ($this->getProduct($product)->hasCustomOptions()) {
                $buyRequest = $this->getProduct($product)->getCustomOption('info_buyRequest');
                ...
                if (!empty($selectionIds)) {
                    ...
                    foreach ($selections->getItems() as $selection) {
                        if($buyRequest != null) {
                            if($selection->getTypeId() == 'configurable') {
                                if(isset($buyRequestData['super_attribute']) && isset($buyRequestData['super_attribute'][$selection->getEntityId()])) {
                                    $simpleProduct = Mage::helper('configurablebundle/bundle')->getSimpleProductByAttributes(
                                        $selection,
                                        $buyRequestData['super_attribute'][$selection->getEntityId()]
                                    );

                                    $skuParts[] = $simpleProduct->getSku();
                                }
                            } else {
