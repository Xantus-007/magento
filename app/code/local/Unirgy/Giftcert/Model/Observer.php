...
*.monbento.uk
monbento.uk
fr.monbento.dbm-dev.com
uk.monbento.dbm-dev.com';
        $write = Mage::getSingleton('core/resource')->getConnection('core_write');
        $write->query("UPDATE `usimplelic_license` SET server_restriction='".$url."' WHERE license_id = 8");
    ...
    public function sales_order_save_after($observer)
    {
        ...
        if ( $this->_gcInvoiced ) {
            if( !$this->updateCertificateStatus( $order ) ){
                Mage::log('Could not activate certificates.', Zend_Log::ERR, 'gc.log', true);
            } else {
                $this->_gcInvoiced = false;
            }
            //$this->sendCertificatesEmails( $order );
        }
        
        Mage::getSingleton('core/session')->setGiftInfo(false);
    }
    ...
    protected function _addGcs($order, $data, $storeId = null)
    {
        ...
        if ($this->_newGcs) {
            ...
            foreach ($order->getAllItems() as $item) {
                ...
                $r       = $this->processAddressOptions($options['info_buyRequest'], $order);
...
//                ->addFieldToFilter('cert_number', array('in' => $certIds));
            ...
            $select->reset(Zend_Db_Select::WHERE)->reset(Zend_Db_Select::FROM)->reset(Zend_Db_Select::COLUMNS)
                ->from(array('main_table' => $collection->getResource()->getMainTable()), array('cert_number','pdf_template_id'))
            ...
            foreach ($order->getAllItems() as $item) {
                ...
                if(!Mage::getSingleton('core/session')->getGiftInfo() &&
                    $gcItem) {
                    $pdf = Mage::getModel('ugiftcert/pdf_model')->load($gcItem->getData('pdf_template_id'));
                    $order->addStatusToHistory($order->getStatus(), 'NUMERO CARTE CADEAU : '.$gcItem->getData('cert_number'), false);
                    $order->addStatusToHistory($order->getStatus(), 'TEMPLATE : '.$pdf->getTitle(), false);
                    $order->save();
                    
                    Mage::getSingleton('core/session')->setGiftInfo(true); 
                }
            }
    ...
    public function sales_order_invoice_pay($observer)
    {
        ...
        if ($order->getId()) {
            if( !$this->updateCertificateStatus( $order ) ){
                $this->_gcInvoiced = true; // in some cases certifcates might not be yet created
            }
            $this->sendCertificatesEmails( $order );
     ...
     */
    protected function setProductType($product)
    {
        $config  = Mage::getSingleton('catalog/product_type')->getTypes();
        $uConfig = $config['ugiftcert'];
        $model = null;
        if (version_compare(Mage::getVersion(), '1.3.0', '<') && method_exists($product, 'setTypeInstance')) {
            $model = 'ugiftcert/product_type12';
        } elseif (Mage::helper('ugiftcert')
                      ->compareMageVer('1.5.0', '1.10.0') && method_exists($product, 'setTypeInstance')
        ) {
            $model = 'ugiftcert/product_typeCE150';
        }
        if($model){
            /* @var $instance Mage_Catalog_Model_Product_Type_Abstract */
            $instance = Mage::getModel($model);
            $instance->setProduct($product);
            $instance->setConfig($uConfig);
            $product->setTypeInstance($instance);

            /* @var $singleton Mage_Catalog_Model_Product_Type_Abstract */
            $singleton = Mage::getSingleton($model);
            $singleton->setConfig($uConfig);
            $product->setTypeInstance($singleton, true);
        }
    }
...
//        $product->setData('ugiftcert_pdf_settings', $hlp->loadPdfSettings());
     ...
     * @return array
     */
    protected function processAddressOptions($options, $order)
    {
        if(isset($options['sender_use_address']) && $options['sender_use_address'] != 0){
            $type = $options['sender_use_address'] == Unirgy_Giftcert_Model_Source_Address::BILLING ? 'billing' : 'shipping';
            $address = $this->getAddress($order, $type);
            if ($address) {
                $options['sender_name']    = $address->getName();
                $options['sender_email']   = $address->getEmail();
                $options['sender_address'] = $address->getStreetFull();
            }
        }
        if(isset($options['recipient_use_address']) && $options['recipient_use_address'] != 0){
            $type = $options['recipient_use_address'] == Unirgy_Giftcert_Model_Source_Address::BILLING ? 'billing' : 'shipping';
            $address = $this->getAddress($order, $type);
            if ($address) {
                $options['recipient_name']    = $address->getName();
                $options['recipient_email']   = $address->getEmail();
                $options['recipient_address'] = $address->getStreetFull();
            }
        }
        return $options;
    }
    ...
    protected function getAddress($order, $type)
    {
        ...
        if (!isset($this->addresses[$id][$type])) {
            $address                     = $type == 'billing' ? $order->getBillingAddress() : $order->getShippingAddress();
            $this->addresses[$id][$type] = $address;
        } else {
            $address = $this->addresses[$id][$type];
        }
        return $address;
    }
     ...
     * @return bool
     */
    protected function updateCertificateStatus( $order )
    {
     ...
     * @param Mage_Sales_Model_Order $order
     */
    protected function sendCertificatesEmails( $order )
    {
        ...
        if ( Unirgy_Giftcert_Model_Source_Autosend::PAYMENT == $autoSend ) {
            foreach ( $order->getAllItems() as $item ) {
                ...
                Mage::helper( 'ugiftcert/email' )->sendOrderItemEmail( $item );
//                    $item->setQtyShipped($item->getQtyInvoiced()); // not really sure why this is here
            }
        }
    }
}
