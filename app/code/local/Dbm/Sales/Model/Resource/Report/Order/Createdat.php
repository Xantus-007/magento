...
class Dbm_Sales_Model_Resource_Report_Order_Createdat extends Mage_Sales_Model_Resource_Report_Order_Createdat{
    ...
    protected function _aggregateByField($aggregationField, $from, $to)
    {
        ...
        try {
            ...
            $selectOrderConfigItem = $adapter->select();
            $selectAttributeCost = $adapter->select();

            $qtyCanceledExpr = $adapter->getIfNullSql('qty_canceled', 0);
            $cols            = array(
                'order_id'           => 'order_id',
                'total_qty_ordered'  => new Zend_Db_Expr("SUM(qty_ordered - {$qtyCanceledExpr})"),
                'total_base_cost'    => new Zend_Db_Expr('SUM(product_cost * qty_ordered)'),
                'total_qty_invoiced' => new Zend_Db_Expr('SUM(qty_invoiced)'),
                'total_qty_invoiced' => new Zend_Db_Expr('SUM(qty_invoiced)'),
            );

            $catalog_product_entity_decimal = Mage::getSingleton('core/resource')->getTableName('catalog_product_entity_decimal');
            $eav_attribute = Mage::getSingleton('core/resource')->getTableName('eav_attribute');

            $selectAttributeCost->from($catalog_product_entity_decimal, array(
                'product_cost' => 'value',
                'product_id' => 'entity_id')
            )->where("attribute_id = (SELECT attribute_id FROM {$eav_attribute} WHERE attribute_code = 'cost' AND entity_type_id = 4)");

            /*$selectOrderItem->from(array('oi' => $this->getTable('sales/order_item')), $cols)
                ->join(array("cost" => $selectAttributeCost), 'cost.product_id = oi.product_id', array())
                ->group('order_id');*/
            ...
            $cols = array(
                ...
                'total_qty_invoiced' => new Zend_Db_Expr('SUM(qty_invoiced)'),
                'total_qty_invoiced' => new Zend_Db_Expr('SUM(qty_invoiced)'),
                'total_base_cost' => new Zend_Db_Expr('SUM(cost * qty_ordered)'),
            );

            $selectOrderConfigItem->from(array('oi' => $this->getTable('sales/order_item')), $cols)
            ...
            $select->from(array('o' => $this->getTable('sales/order')), $columns)
                ->join(array('oi' => $selectOrderConfigItem), 'oi.order_id = o.entity_id', array())
