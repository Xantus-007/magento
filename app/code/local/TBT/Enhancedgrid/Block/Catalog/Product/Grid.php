 ...
 */
class TBT_Enhancedgrid_Block_Catalog_Product_Grid extends Dbm_Country_Block_Rewrite_AdminCatalogProductGrid {
    ...
    protected function _prepareCollection() {
        ...
        $this->_prepareStockCollection($collection);
        ...
        $this->loadEavColumn( $collection );
        
        
        if($store->getId() || $website->getId())
        {
            ...
            /*elseif($website->getId())
            {
                $collection->addWebsiteFilter($website->getId());
            }*/
            
            $collection->joinAttribute( 'tbtcustom_name', 'catalog_product/name', 'entity_id', null, 'inner', $store->getId() );
            $collection->joinAttribute( 'tbtstatus', 'catalog_product/status', 'entity_id', null, 'inner', $store->getId() );
            $collection->joinAttribute( 'tbtvisibility', 'catalog_product/visibility', 'entity_id', null, 'inner', $store->getId() );
            $collection->joinAttribute( 'tbtprice', 'catalog_product/price', 'entity_id', null, 'left', $store->getId() );
            $collection->joinAttribute( 'tbtspecial_price', 'catalog_product/special_price', 'entity_id', null, 'left', $store->getId() );
        } else {
    ...
    {
        $siteId = $this->getRequest()->getParam( 'store', 0 );
        list($prefix, $siteId) = explode('_', $siteId);
        
        $site = Mage::app()->getWebsite($siteId);
        if($site->getId() != $siteId) $site = Mage::app()->getStore(0);
    ...
    protected function _addColumnFilterToCollection($column) {
        if ( $this->getCollection() ) {
            if ( $column->getId() == 'websites' && $this->_iterator < 1) {
    ...
    public function loadEavColumn(&$collection){
        $ignoreCols = array(
            'id' => true, 
            'websites' => true, 
            'status' => true, 
        ...
        foreach ($this->columnSettings as $col => $true) {
            ...
            $attr = Mage::getModel('eav/entity_attribute')->loadByCode('catalog_product', $col);
            $collection->addAttributeToSelect( $attr->getName() );
        }
    }
    ...
    protected function _prepareColumns() {
        ...
        //if ( $this->colIsVisible( 'qty' ) ) {
            $this->addColumn( 'qty_final', 
                array(
                    'header' => Mage::helper( 'catalog' )->__( 'Qty' ).' (défaut)', 
                    'width' => '100px', 
                    'type' => 'number', 
                    'index' => 'qty_final'
                ) );
        ...
        foreach($websites as $website)
        {
            $this->addColumnAfter('status_'.$website->getCode(),
                array(
                    ...
                    'options' => array(NULL => '-') + Mage::getModel( 'catalog/product_status' )->getOptionArray(),
            ), 'status');
            ...
            $this->addColumnAfter('qty_'.$website->getCode(),
                array(
                    ...
                    //'filter_index' => 'qty_'.$website->getCode(),
            ), 'qty_final');
        }
    ...
    protected function _addVariableColumns() {
        ...
        foreach ($this->columnSettings as $col => $true) {
            ...
            Mage::log(print_r($innerSettings, true));
    ...
    protected function _prepareStockCollection($collection)
    {
        $store = $this->_getStore(); // aitoc code
        $websiteIds = array(1 => 'eu', 3 => 'us');
        ...
        $collection->joinField('qty_final',
...
//                '{{table}}.stock_id=1',
            ...
            $collection->joinField('qty_default_'.$website->getCode(),
                'aitquantitymanager/stock_item',
            ...
            $collection->getSelect()->columns(new Zend_Db_Expr('IF(`_table_qty_default_'.$website->getCode().'`.`use_default_website_stock` = 1, " -- ", `_table_qty_'.$website->getCode().'`.`qty`) as qty_'.$website->getCode()));
        }
