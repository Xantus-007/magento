 ...
 */
?>
<?php if(Aitoc_Aitsys_Abstract_Service::initSource(__FILE__,'Aitoc_Aitpermissions')){ hpkmYyDZcZhCiirw('d76c96f3fb3071fdff85ef4c9fa96180'); ?><?php
 ...
 */
    public function onCatalogProductSaveAfter($observer)
    {
        ...
        if (!(Mage::getSingleton('catalog/session')->getIsNewProduct(true)) &&
            !('catalog_product' == $controllerName && 'quickCreate' == $actionName))
        {
            return;
        }
        ...
        if (!('catalog_product' == $controllerName && 'quickCreate' == $actionName))
        {
            $this->insertAttributeValue(
                $attributeTable,
                $productEntityTypeId,
                $createdByAttributeId,
                0,
                $product->getId(),
                $adminId
            );
