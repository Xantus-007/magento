...
/**
* 	This class is basically a magento wrapper for the VadsApi class + some useful model methods
*/
class Mage_Vads_Model_Api_Standard extends Varien_Object
{
	...
	public function Mage_Vads_Model_Api_Standard()
	{
		$this->vads_api = new VadsApi();
	}
	...
	public function getConfigArray($name="")
	{
		...
		$xml_node = 'global/payment/vads/'.$name;
		foreach(Mage::getConfig()->getNode($xml_node)->asArray() as $xml_data)
		{
			$result[$xml_data['name']] = $xml_data['code'];
		}
		return $result;
	}
	...
	public function set($name='',$value=null){
		return $this->vads_api->set($name,$value);
	}
	
	public function get($name=''){
		return $this->vads_api->get($name);
	}
	
	public function setFromArray($params){
		return $this->vads_api->setFromArray($params);
	}
	
	public function getRequestHtmlForm($enteteAdd='',$inputType='hidden',
										$buttonValue='Aller sur la plateforme de paiement',
										$buttonAdd='',$buttonType='submit')	{
		return $this->vads_api->getRequestHtmlForm($enteteAdd,$inputType,$buttonValue,$buttonAdd,$buttonType);
	}
	
	public function getResponse($tab=null, $ctx_mode=null, $key_test=null, $key_prod=null){
		return $this->vads_api->getResponse($tab, $ctx_mode, $key_test, $key_prod);
	}
	
	public function getCurrencyNum($alpha3) {
		$ret = $this->vads_api->findCurrencyByAlphaCode($alpha3);
		if ($ret != null ){
		  	return $ret->num ;
		} else {
			return null;
