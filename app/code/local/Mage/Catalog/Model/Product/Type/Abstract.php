...
abstract class Mage_Catalog_Model_Product_Type_Abstract
{
    ...
    public function isSalable($product = null)
    {
        ...
        $qty = Mage::getModel('cataloginventory/stock_item')->loadByProduct($this->getProduct($product))->getQty();
        if ($salable && $this->getProduct($product)->hasData('is_salable')) {
            $productType = $this->getProduct($product)->getTypeId();
            $salable = ($qty > 0 || $productType != Mage_Catalog_Model_Product_Type::TYPE_SIMPLE) ? $this->getProduct($product)->getData('is_salable') : 0;
            if($productType == Mage_Catalog_Model_Product_Type::TYPE_BUNDLE && !Mage::registry('product')) $salable = true;
        }
        elseif ($salable && $this->isComposite()) {
            $salable = null;
        }

        if ($salable && $this->isComposite() && $this->getProduct($product)->getModulePersonnalisation() !== '1') {
            $salable = null;
        }

        return (is_null($salable)) ? $salable : (boolean) (int) $salable;
    }
