...
class Mage_Bundle_Model_Sales_Order_Pdf_Items_Invoice extends Mage_Bundle_Model_Sales_Order_Pdf_Items_Abstract
{
    ...
    public function draw()
    {
        ...
        $percentTax = round((($totalPrice_tax / $item->getRowTotal() - 1) * 100),0); 
        ...
        foreach ($items as $_item) {
            ...
            if (!$_item->getOrderItem()->getParentItem()) {
                $text = array();
                foreach ($stringHelper->str_split($item->getSku(), 14) as $part) {
                    $text[] = $part;
                }
                $line[] = array(
                    'text'  => $text,
            ...
            if ($this->canShowPriceInfo($_item)) {
                ...
                $line[] = array(
                    'text'  => $order->formatPriceTxt($price_withoutTax),
                    'feed'  => 300,
                    'font'  => 'bold'
                );
                $line[] = array(
                ...
                if($item->getTaxAmount() >= 0) {
                    // draw Tax
                    $line[] = array(
                        'text'  => $order->formatPriceTxt($totalTax),
                        'feed'  => 415,
                        'font'  => 'bold'
                    );
