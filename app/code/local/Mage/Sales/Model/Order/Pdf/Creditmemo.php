...
class Mage_Sales_Model_Order_Pdf_Creditmemo extends Mage_Sales_Model_Order_Pdf_Abstract
{
    public function getPdf($creditmemos = array())
    {
        ...
        foreach ($creditmemos as $creditmemo) {
            ...
            if ($order->hasInvoices())
            {
                ...
                {
                    $invoice = $value->getIncrementId();
                    break;
                }
            }
            $page->drawText(Mage::helper('sales')->__('Invoice # ') . $invoice, 250, 770, 'UTF-8');

            /* Add credit memo created date */
            $creditmemoCreatedAt = Mage::helper('core')->formatDate($creditmemo->getCreatedAt(), 'medium', false);
            $page->drawText(Mage::helper('sales')->__('Credit Memo created at : ') . $creditmemoCreatedAt, 250, 760, 'UTF-8');
            ...
            foreach ($creditmemo->getAllItems() as $item){
                ...
                $percentTax = round((($item->getRowTotalInclTax() / $item->getRowTotal() - 1) * 100),0);
                $price_withoutTax = $item->getRowTotal() / $item->getQty();
                $totalTax = $item->getRowTotal() * $percentTax / 100;
                $totalPrice_tax = $item->getRowTotal() + $totalTax;
                
                $totauxCustom[] = array("price_ht" => $item->getRowTotal(), "TVA" => $totalTax, "Rate" => $percentTax);
            }

            /* Add totals */
            $page = $this->insertTotals($page, $creditmemo, $totauxCustom, true);
        }
    ...
    protected function _drawHeader(Zend_Pdf_Page $page)
    {
        ...
        $page->drawText(Mage::helper('sales')->__('Products refund'), $x = 35, $this->y, 'UTF-8');
        $x += 220;
