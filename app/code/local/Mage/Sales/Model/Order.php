...
class Mage_Sales_Model_Order extends Mage_Sales_Model_Abstract
{
    ...
    {
        $testGiftCert = true;
        if(!$this->getIsNotVirtual()) {
            $items = $this->getAllItems();
            foreach($items as $item){
                $product = Mage::getModel('catalog/product')->load($item->getProductId());
                if($product->getTypeID() == 'ugiftcert') {
                    $options = $item->getProductOptions();
                    if($options['info_buyRequest']['delivery_type'] == 'physical') $testGiftCert = false;
                }
            }
        }
        
        if($testGiftCert || $state != 'complete') {
        ...
        Mage::log('-------------START-------------', null, 'orderstatus.log');
        Mage::log('ORDER : ' . $this->getId() . ' | STATE : ' . $state . ' | STATUS : ' . $status, null, 'orderstatus.log');
        if ($status) {
           Mage::log('COMMENT : ' . $comment, null, 'orderstatus.log'); 
        }
        Mage::log(debug_backtrace(2), null, 'orderstatus.log');
        Mage::log('--------------END--------------', null, 'orderstatus.log');
        return $this;
    }
    ...
    public function queueNewOrderEmail($forceMode = false)
    {
        ...
        {
            $templateId = Mage::getStoreConfig('monbento_kiosk/general/template', $storeId);
        }
