...
class Mage_Tax_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function getPrice($product, $price, $includingTax = null, $shippingAddress = null, $billingAddress = null,
                             $ctc = null, $store = null, $priceIncludesTax = null, $roundPrice = true, $shippingBasedOn = false)
    {
        if (!$price) {
            return $price;
        }
        ...
        if (is_null($percent)) {
            if ($taxClassId) {
                $request = Mage::getSingleton('tax/calculation')
                    ->getRateRequest($shippingAddress, $billingAddress, $ctc, $store, $shippingBasedOn);
        ...
        if ($taxClassId && $priceIncludesTax) {
            if ($this->isCrossBorderTradeEnabled($store)) {
                ...
                $request = Mage::getSingleton('tax/calculation')->getRateOriginRequest($store, $shippingBasedOn);
    ...
    public function getShippingTaxBasedOn($store = null)
    {
        return Mage::getStoreConfig(Mage_Tax_Model_Config::CONFIG_XML_PATH_SHIPPING_BASED_ON, $store);
    }
