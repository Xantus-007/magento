...
class Mage_Checkout_Model_Type_Onepage
{
    ...
    public function saveBilling($data, $customerAddressId)
    {
        ...
        Mage::log('STARING TOTALS', null, 'api.xml');
        Mage::log('TOTALS BEFORE01 : '.$this->getQuote()->getGrandTotal(), null, 'api.xml');
        $this->getQuote()->collectTotals();
        Mage::log('TOTALS BEFORE02 : '.$this->getQuote()->getGrandTotal(), null, 'api.xml');
        $this->getQuote()->setTotalsCollectedFlag(false);                                //Rajout Ludo
        Mage::log('TOTALS BEFORE03 : '.$this->getQuote()->getGrandTotal(), null, 'api.xml');
        $this->getQuote()->getShippingAddress()->setCollectShippingRates(true)->save(); //Rajout Ludo
        Mage::log('TOTALS BEFORE04 : '.$this->getQuote()->getGrandTotal(), null, 'api.xml');
        $this->getQuote()->collectTotals();                                              //Rajout Ludo
        Mage::log('TOTALS BEFORE05 : '.$this->getQuote()->getGrandTotal(), null, 'api.xml');
        $this->getQuote()->save();
        Mage::log('TOTALS BEFORE06 : '.$this->getQuote()->getGrandTotal(), null, 'api.xml');
    ...
    protected function _prepareNewCustomerQuote()
    {
        ...
        $passwordCreatedTime = $this->_checkoutSession->getData('_session_validator_data')['session_expire_timestamp'] - Mage::getSingleton('core/cookie')->getLifetime();
