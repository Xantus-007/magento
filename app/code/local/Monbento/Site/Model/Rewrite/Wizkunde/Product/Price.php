...
class Monbento_Site_Model_Rewrite_Wizkunde_Product_Price extends Mage_Bundle_Model_Product_Price {
                                                ...
                                                $multiplyQty = true, $takeTierPrice = true)
    {
            ...
            if($buyRequest != null) {
                if($selectionProduct->getTypeId() == 'configurable') {
                    if(isset($buyRequestData['super_attribute']) && isset($buyRequestData['super_attribute'][$selectionProduct->getEntityId()])) {
                        if($selectionId = $selectionProduct->getSelectionId())
                        {
                            $optionsBySelection = array_flip($buyRequestData['bundle_option']);
                            $optionId = $optionsBySelection[$selectionId];
                            $productId = $buyRequestData['simple_selection']['bundle-option-'.$optionId];
                            $loadedProduct = Mage::getModel('catalog/product')->load($productId);
                        }
                        elseif($selectionId = $selectionProduct->getCustomOption('selection_id')->getValue())
                        {
                            $optionsBySelection = array_flip($buyRequestData['bundle_option']);
                            $optionId = $optionsBySelection[$selectionId];
                            $productId = $buyRequestData['simple_selection']['bundle-option-'.$optionId];
        ...
        if($bundleProduct->getModulePersonnalisation() != 1) return $price;
