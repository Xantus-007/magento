...
require_once(Mage::getModuleDir('controllers', 'Belvg_FacebookFree') . DS . 'CustomerController.php');

class Monbento_Site_Rewrite_Belvg_CustomerController extends Belvg_FacebookFree_CustomerController
{
    ...
    public function LoginAction()
    {
        ...
        $me = json_decode($this->getFbData('https://graph.facebook.com/me?fields=id,name,email&access_token=' . $cookie['access_token']));
        Mage::log('ACCESS TOKEN : ' . $cookie['access_token']);
        if (!is_null($me)) {
            $me = (array) $me;
    ...
    private function _loginPostRedirect(&$session)
    {
        ...
        if ($referer = $this->getRequest()->getParam(Mage_Customer_Helper_Data::REFERER_QUERY_PARAM_NAME)) {
            ...
            if ((strpos($referer, Mage::app()->getStore()->getBaseUrl()) === 0) || (strpos($referer, Mage::app()->getStore()->getBaseUrl(Mage_Core_Model_Store::URL_TYPE_LINK, true)) === 0)) {
    ...
    private function get_new_facebook_cookie($app_id, $app_secret)
    {
        ...
        if (!is_null($signed_request)) {
            ...
            $response = json_decode($access_token_response, true);
            $signed_request['access_token'] = $response['access_token'];
            $signed_request['expires'] = time() + $response['expires_in'];
        }
