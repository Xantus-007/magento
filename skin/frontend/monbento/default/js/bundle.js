...
function changeOption(el, delayreload) {
    ...
    if (el.children('span:not(.hover)').attr('rel'))
        changeCustomImage(exploded[1], el.children('span:not(.hover)').attr('rel'), delayreload);
}
...
function changeCustomImage(el, url, noreload) {
    jQuery('#element' + el + ', #sticky_element' + el).fadeTo(100, 0, function ()
    {
...
function changeFBImage() {
    ...
    var addthisUrl = window.location;
    datas = {};
    jQuery('.c-colorSelector .bundle-color.active').each(
            function (intIndex) {
                var exploded = jQuery(this).attr('rel').split(',');
                datas[exploded[1]] = exploded[2];
                var option = jQuery(this).parent().find('input').attr('id').split('-');
                addthisUrl = addthisUrl + 'bundle-option-' + option[2] + '=' + exploded[0] + '&'
            }
    );
    ...
    jQuery.ajax({
        ...
        window.history.pushState('updateBentoPerso-'+customHistoryName.getTime(), 'test', addthisUrl);
    });
}
...
function changeDownloadLinkImage() {
    ...
    jQuery('.c-controls .js-controlsPersonnalize__download').attr('href', query + '/' + datas);
}
...
(function ($) {
    $(document).ready(function () {
        if($('.c-colorSelector__item .bundle-color.active').size() > 0)
        {
            changeDownloadLinkImage();
        }
        
        $('.c-row__personnalize').on('click', '.bundle-color', function () {
            changeOption($(this));
        });
        ...
        jQuery(function ($) {
            ...
            if($('.bundle-color.active').size() > 0) {
                $.each($('.bundle-color.active'), function() {
                    $(this).trigger( "click" );
                });
            }
        });
